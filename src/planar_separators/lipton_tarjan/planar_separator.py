...
def _map_levels(new_vertices_count, new_vertices_mapping, bfs_levels):
    ...
    new_bfs_levels = utils.repeat_int(-1, new_vertices_count)
...
def _get_new_vertices_mappings_composition(new_vertices_mapping1, new_vertices_mapping2):
    ...
    composition = utils.repeat_int(-1, len(new_vertices_mapping1))
...
def _map_edges_mask(new_edges_count, new_edge_indices_mapping, edges_mask):
    ...
    new_edges_mask = utils.repeat_bool(False, new_edges_count)
...
def _get_new_parent_edge_indices(parent_edge_indices, new_edge_indices_mapping):
    ...
    new_parent_edge_indices = utils.repeat_int(-1, len(parent_edge_indices))
...
def _mark_separation_when_components_less_than_one_third(connected_component_indices,
    ...
    separation = utils.repeat_int(separation_class.SECOND_PART, len(connected_component_indices))
...
def _mark_separation_if_cost_between_levels_is_less_than_two_thirds(bfs_levels,
    ...
    level_separation = utils.repeat_int(separation_class.UNDEFINED, levels_count)
    ...
    separation = utils.repeat_int(separation_class.UNDEFINED, len(bfs_levels))
...
def _mark_separation_of_graph_with_small_radius(graph, center_vertex):
    ...
    if graph.size < 3:
        return utils.repeat_int(separation_class.SEPARATOR, graph.size)
    ...
    separation = utils.repeat_int(separation_class.UNDEFINED, graph.size)
...
def _mark_separation_for_one_connected_component(graph, connected_component_indices,
    ...
    new_vertices_mapping1, _, new_graph = planar_graph_constructor.construct_subgraph(graph,
            vertices_below_level_two_mask, utils.repeat_bool(True, graph.edges_count))
...
def mark_separation(graph):
    ...
    connected_component_indices = utils.color_connected_components(graph)
