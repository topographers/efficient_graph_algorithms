...
def iterate_tree_adjacency_costs_on_tree_cycle_side(graph, tree_edges_mask,
    ...
    for edge_index in utils.iterate_subgraph_incidence_indices(graph, cycle_edges_mask,
            tree_edges_mask, start_vertex_on_cycle, start_edge_index_on_cycle):

        cycle_vertex = _get_cycle_vertex(edge_index, graph, cycle_vertices_mask)
...
def shrink_cycle(graph, tree_edges_mask, total_descendants_costs, parent_edge_indices,
    ...
    if tree_edges_mask[first_internal_edge_index] or \
        ...
        return _shrink_cycle_if_one_of_internal_edges_is_on_tree(graph.vertex_costs,
    ...
    tree_path_cost = _add_tree_path_to_cycle_masks_and_return_its_cost(graph,
    ...
    adjacency_costs_inside_first_internal_cycle = \
            iterate_tree_adjacency_costs_on_tree_cycle_side(graph, tree_edges_mask,
