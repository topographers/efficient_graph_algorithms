...
def get_rgb(input_arr: np.ndarray):
    """
...
def simple3d_save_gif(distribution: np.ndarray, mesh_data: MeshData, output_file_path: str):
    """
    input: distribution is a 1d array, length is equal to the number of vertices

    save the plot as a gif file
    """
    rgb_colors = get_rgb(distribution)
    input_colors = np.ones(((len(mesh_data.vertices)), 4))
    input_colors[:, 0:3] = rgb_colors
    scaled_vertices = mesh_data.vertices / (np.max(mesh_data.vertices) - np.min(mesh_data.vertices))
    m = Mesh.from_faces(scaled_vertices, mesh_data.faces, colors=input_colors)
    render(
        m,
...
def plot_mesh(world_pos: np.ndarray,
    ...
    interpolated_velocities = go.Scatter3d(x=vx_interpolated, y=vy_interpolated, z=vz_interpolated,
                                           mode='lines', name='interpolated_velocities',
